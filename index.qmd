...
I have included all code to my three different visualizations below! All plots were generated using `ggplot()` and I used `magick()` to add these plots together create a base infographic.When utilizing   `magick()`, some text was rendering fuzzy, despite being clear in the inidvidual PNGs. To make text as legible as possible, I ended up individually adding the PNGs to a Canva document. I used the base infographic that I created below ( with the background color and title) as the background, and then added all other elements on top in Canva. I hope you enjoy exploring my infographic and code below!
...
library(png)
library(grid)
img <- readPNG('output/infographic.png')
grid.raster(img)
